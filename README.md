...
│       ├── stream_mappers.py  # Преобразование SDK stream responses в domain DTO
│       └── ...            # Дополнительные файлы (например, тесты, лог-файлы, вспомогательные JSON с инструментами)
├── domain/                # Чистые доменные DTO и бизнес-правила без привязки к SDK/API
│   ├── stream_events.py   # DTO событий стрима: LastPriceEvent, PortfolioSnapshotEvent и др.
│   └── signals.py         # Правила принятия торговых сигналов
├── core/                  # Ядро системы и бизнес-логика
│   ├── domains/           # Доменные сущности и события
│   │   ├── message_bus.py       # Контракт шины событий для клиентов, API и воркеров
│   │   ├── event_bus.py         # In-memory реализация для тестов и локальной отладки
│   │   ├── redis_stream_bus.py  # Redis Streams реализация для межпроцессного обмена
│   │   └── event_codec.py       # JSON-envelope сериализация событий в Redis payload
│   └── schemas/           # Логика обработки входящих данных (схемы/процессоры)
│       └── stream_processor.py  # Класс MarketDataProcessor: подписывается на события MessageBus и реагирует на рыночные данные (LastPrice, свечи, и т.п.), генерирует торговые сигналы и уведомления
...
clients/tinkoff/… – отвечает за взаимодействие с API Тинькофф. Класс TClient инкапсулирует логину/подключение к API, получение данных (аккаунты, портфель, избранное, исторические свечи) и управление стримингом цен. Stream responses SDK преобразуются в собственные domain DTO в stream_mappers.py до публикации в MessageBus. Дальше по системе не должны ходить объекты SDK стрима.

core/… – содержит обобщенную логику обработки данных. MessageBus передаёт domain-события от Tinkoff adapter к обработчикам. По умолчанию используется Redis Streams, поэтому Telegram, будущий FastAPI и отдельные воркеры могут работать разными процессами и обмениваться событиями через Redis. In-memory StreamBus оставлен как backend для тестов и локальной отладки. MarketDataProcessor получает события (например, LastPriceEvent) и, опираясь на данные из БД (последние рассчитанные уровни Дончиана, ATR, флаги состояний), принимает решения: нужно ли отправить сигнал пользователю, нужно ли обновить статус позиции и т.п.

События в Redis сериализуются как JSON envelope с полями `event_type`, `version`, `payload`. Поэтому stream можно читать из FastAPI, отдельных Python-воркеров или другого сервиса без зависимости от классов Tinkoff SDK.

Для Redis Streams одна consumer group распределяет события между воркерами. Если нескольким сервисам нужны одни и те же события независимо друг от друга, задавайте им разные `message-bus.group`. Для распределения нагрузки между одинаковыми воркерами используйте одну группу и разные `message-bus.consumer`. Для восстановления pending-сообщений после рестарта лучше задавать стабильное имя `consumer` для конкретного процесса.
...
Создание MarketDataProcessor и подписка его на события стрима (`MessageBus.subscribe('market_data_stream', processor.execute)`).

Настройка APScheduler: добавление заданий на открытие/закрытие рынка согласно Config.

Запуск основных служб: redis.connect(), stream_bus.start(), scheduler.start(), выполнение первоначальной задачи открытия (_job_open_if_needed) и обновления индикаторов, после чего – запуск бесконечного цикла dp.start_polling() для обработки сообщений Telegram.

Корректное завершение работы (при остановке программы) через service.stop(): останавливается планировщик, закрывается стрим Tinkoff, завершается сессия Telegram Bot API, отключается шина событий и закрывается Redis.
